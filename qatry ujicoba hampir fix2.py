...
import context_lokasi_keperluan
...
penting_contexts = context_lokasi_url.petunjuk_lokasi #context berisi lokasi,patokan, dan url
cari_contexts = context_lokasi.deskripsi_lokasi #context berisi deskripsi dan keterangan lokasi
keperluan_contexts = context_lokasi_keperluan.petunjuk_keperluan #context berisi deskripsi dan keterangan lokasi
...
def start(update: Update, context: CallbackContext) -> None:
    keyboard = [
        [InlineKeyboardButton("Penting", callback_data='penting')],
        [InlineKeyboardButton("Keperluan", callback_data='keperluan')],
...
def button_click(update: Update, context: CallbackContext) -> None:
    ...
    if choice == 'cancel':  # Menambahkan pilihan 'cancel'
        ...
        if choice == 'penting':
            if update.callback_query.message.text != 'Anda telah memilih opsi "Penting". Silakan ajukan pertanyaan Anda:':
                update.callback_query.edit_message_text(text='Anda telah memilih opsi "Penting". Silakan ajukan pertanyaan Anda:')
        else:
        ...
        if choice == 'keperluan':
            if update.callback_query.message.text != 'Anda telah memilih opsi "Keperluan". Silakan ajukan pertanyaan Anda:':
                update.callback_query.edit_message_text(text='Anda telah memilih opsi "Keperluan". Silakan ajukan pertanyaan Anda:')
...
def start_penting(update: Update, context: CallbackContext) -> None:
    context.user_data['choice'] = 'penting'
    update.message.reply_text("Penting kemana yang ingin Anda tuju?")

def start_keperluan(update: Update, context: CallbackContext) -> None:
    context.user_data['choice'] = 'keperluan'
    update.message.reply_text("Keperluan kemana yang ingin Anda tuju?")
def penting_command(update: Update, context: CallbackContext) -> None:
    update.message.reply_text('Anda memilih opsi "Penting". Silakan ajukan pertanyaan Anda:')
    context.user_data['choice'] = 'penting'
    if 'cari_topik' in context.user_data:
        del context.user_data['cari_topik']  # Hapus topik penjelasan saat berpindah ke penting lokasi

def keperluan_command(update: Update, context: CallbackContext) -> None:
    update.message.reply_text('Anda memilih opsi "Keperluan". Silakan ajukan pertanyaan Anda:')
    context.user_data['choice'] = 'keperluan'
    if 'cari_topik' in context.user_data:
        del context.user_data['cari_topik']  # Hapus topik penjelasan saat berpindah ke keperluan lokasi
...
def extract_info_from_context_kebutuhan(context):
    url_matches = re.findall(r"<url>\s*(.*?)\s*</url>", context)
    lati_matches = re.findall(r"<lati>\s*(.*?)\s*</lati>", context)
    longi_matches = re.findall(r"<longi>\s*(.*?)\s*</longi>", context)
    kebutuhan_matches = re.findall(r"<kebutuhan>\s*(.*?)\s*</kebutuhan>", context)
    return url_matches, lati_matches, longi_matches, kebutuhan_matches
...
def answer_question(update: Update, context: CallbackContext) -> None:
    ...
    if not context.user_data.get("choice"):
        update.message.reply_text("Mohon pilih 'penting' atau 'Cari' terlebih dahulu sebelum mengajukan pertanyaan.")
        return

    if context.user_data["choice"] == "penting":
        best_context = choose_context(message, penting_contexts)
        print(f"Best context: {best_context}")
        if best_context is None:
            update.message.reply_text("Maaf, saya tidak dapat menemukan lokasi penting yang Anda cari.")
            return
        ...
        for i in range(len(url)):
            ...
            answer = f"map lokasi penting yang anda cari: {translations[i]}\n\n https://www.google.com/maps/search/{location_name}"
    ...
    if context.user_data["choice"] == "keperluan":
        best_context = choose_context(message, keperluan_contexts)
        print(f"Best context: {best_context}")
        if best_context is None:
            update.message.reply_text("Maaf, saya tidak dapat menemukan lokasi yang anda perlukan.")
            return
        url, lati, longi, kebutuhan= extract_info_from_context_kebutuhan(best_context)
        for i in range(len(url)):
            chat_id = update.effective_chat.id
            latitude = lati[0]
            longitude = longi[0]
            location_name = url[0].replace(" ", "+")
            answer = f"map lokasi keperluan yang anda cari: {kebutuhan[i]}\n\n https://www.google.com/maps/search/{location_name}"
            answer_map = context.bot.send_location(chat_id=chat_id,latitude=latitude, longitude=longitude)
            update.message.reply_text(answer) & message(answer_map)
...
def main() -> None:
    ...
    dispatcher.add_handler(CommandHandler("penting", penting_command))
    dispatcher.add_handler(CommandHandler("keperluan", keperluan_command))
    dispatcher.add_handler(CommandHandler("cari", cari_command))
    dispatcher.add_handler(CallbackQueryHandler(button_click, pattern='^penting$|^cari$'))
    dispatcher.add_handler(CallbackQueryHandler(button_click, pattern='^keperluan$|^cari$'))
